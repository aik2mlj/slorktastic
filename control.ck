            ...
            while( trak.recv( msg ) )
            {
                ...
                {            
                    ...
                    if( msg.which >= 0 && msg.which < 6 )
                    {
                        ...
                        // save last
                        SMOOTHING_FACTOR * velocity[msg.which] + (1.0 - SMOOTHING_FACTOR) * (axis[msg.which] - lastAxis[msg.which] ) * (1000) => velocity[msg.which];
                        // (axis[msg.which] - lastAxis[msg.which] ) * (1000) => velocity[msg.which];
                        axis[msg.which] => lastAxis[msg.which];
                        // chout <= "velocity: " <= "x = " <= velocity[3] <= " y = " <= velocity[4] <= " z = " <= velocity[5] <= IO.newline();

                        // chout <= "x: " <= axis[0] <= " y: " <= axis[1] <= " z: " <= axis[2] <= IO.newline();
